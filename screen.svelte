...
<script>
	...
	import { client } from './_apollo';                //check
  ...
  var update = () => {
    window.document.getElementById("now").innerHTML = moment().format("h:mm:ss");
    window.document.getElementById("period").innerHTML = moment().format("A");
  };
